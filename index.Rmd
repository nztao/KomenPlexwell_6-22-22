...
-   organ/anatomy (*i.e. heart, brain, cortex, midbrain, hippocampus*)
...
1.  **Komen re-runs for the 5-3-22 experiment** (_n = 123_)
2.  **Rachel Morgan** (_n = 38_)
3.  **Katelyn Polemi** (_n = 13_)
4.  **chirexp** (_n = 12_)
5.  **Linda Samuelson** (_n = 6_)
...
-   Starts with "*5Y \| 6\_*"
-   Contains "*Heart \| Brain \| Cortex \| Midbrain \| Hippoc*"

3.  **Katelyn Polemi**

-   Contains "*MCF10A_ML | MCF10A_MS | MCF10A_HG*"
-   Starts with "*MCF7*"

4.  **chirexp**

-   Contains "*MCF10A-DMSO 1 | MCF10A-DMSO 2 | MCF10A-DMSO 3 | MCF10A-CHIR*"

5.  **Linda Samuelson**

-   Ends with "*V1 \| D1 \| V2 \| D2 \| V3 \| D3*"
...
#Split the all.metadata dataframe by filter & grep w/ multiple regex patterns
  split.metadata.list <- list(
    ...
    filter(all.metadata, 
           grepl(paste(c("7518","7889", "7953","8195", "8519", "8580"), 
                       collapse = "|"), all.metadata$Description)),
    
    #Rachel Morgan samples
    #Note: You need to add regex symbol'^' for string starts with
    filter(all.metadata, 
           grepl(paste(c("Heart", "Hippoc", "Brain","Cortex", "Midbrain", "^5Y", "^6_"), 
                       collapse = "|"), all.metadata$Description)),
    
    #Katelyn Polemi samples
    filter(all.metadata,
           grepl(paste(c("MCF10A_ML", "MCF10A_MS", "MCF10A_HG", "^MCF7"), 
                       collapse = "|"), all.metadata$Description)),
    
    #chirexp samples
    filter(all.metadata,
           grepl(paste(c("MCF10A-DMSO 1", "MCF10A-DMSO 2", "MCF10A-DMSO 3", "MCF10A-CHIR"), 
                       collapse = "|"), all.metadata$Description)),    
    #Linda Samuelson
    #Note: You need to add regex symbol `$` for string ends with 
    filter(all.metadata,
           grepl(paste(c("V1$", "D1$", "V2$", "D2$", "V3$", "D3$"), 
                       collapse = "|"), all.metadata$Description))
      )
...
Perform a sanity to check to see that all `all.metadata$Description` entries were properly included in the data splits

```{r eval=FALSE}
#Number of entries per metadata split 
sapply(split.metadata.list, nrow)

#Make sure all entries were included from the raw metadata
sum(sapply(split.metadata.list, nrow)) == nrow(all.metadata)
```
...
all(split.metadata.list$chirexp$sample.id == colnames(split.counts$chirexp))
all(split.metadata.list$lindas.metadata$sample.id == colnames(split.counts$lindas.metadata))

#Make sure all entries in metadata and counts splits match
sum(sapply(split.metadata.list, nrow)) == sum(sapply(split.counts, ncol))
```
...
```{r MDS, fig.cap="Multidimensional scaling plot (MDS) (**_left_**) and scree barplot (**_right_**) of log2-CPM values over default dimensions 1 and 2. Principle Coordinate Analysis was performed using the Euclidean distance metric for the average (_i.e. root-mean-square_) log2-fold changes of the top 500 genes divergent between pairs of samples.", fig.wide=TRUE, fig.show='asis', results='asis', echo=TRUE}
